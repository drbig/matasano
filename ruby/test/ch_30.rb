...
class TestChallenge30 < Minitest::Test
  def setup
    @key = Cryptopals.random_ascii(16)
  end

  def sign(msg)
    Cryptopals::CHash.md4(@key + msg)
  end

  def verify(msg, sig)
    sig == Cryptopals::CHash.md4(@key + msg)
  end
  ...
  def test_ch_29
    ...
    ks = 6.upto(16) do |i|
      si = i + m.length
      p = Cryptopals::CHash.md4_pad(si)
      am = m + p + t
      as = Cryptopals::CHash.md4(t, :from => s, :size => si)
